	...
	async function parseRemote(): Promise<void> {
		...
		try {
			const res = await fetch(
				`/api/read?strict=${strictMode ? "1" : "0"}`,
				{
					method: "POST",
	...
	<div class="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-6">
		...
		>
			...
			>
				...
				Stammdaten, Befundzeilen und eine annotierte Zeilendarstellung
		...
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:items-stretch">
			...
			>
				<CardHeader>
					...
					<CardDescription
						>FK&nbsp;3000, 3101, 3102, 3103, 3110 &mdash;
		...
		<Card
			...
			>
				<CardHeader class="border-border/60 border-b pb-4">
					...
					<CardDescription
						>Binärneutraler latin1‑Text&nbsp;&mdash;&nbsp;hier
						bearbeitbar</CardDescription
					>
				</CardHeader>
