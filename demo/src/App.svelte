...
<div class="text-foreground relative min-h-svh bg-muted/40 px-5 py-8 font-sans">
	<div class="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-6">
		...
		>
			<p class="text-primary text-xs font-semibold tracking-wide uppercase md:text-sm">
				Medizinisches Datenaustauschformat
			</p>
			<h1 class="text-foreground text-balance text-3xl font-semibold tracking-tight md:text-4xl">
				GDT Parser &mdash; strukturierte Vorschau
			</h1>
			<p class="text-muted-foreground max-w-3xl text-[0.95rem] leading-relaxed md:text-base">
				Laden oder bearbeiten Sie eine GDT-Datei (<span class="font-mono text-xs">latin1 / ISO-8859-1</span>). Der
				Server validiert Fremdschlüssel, die Oberfläche zeigt Stammdaten, Befundzeilen und eine annotierte
				Zeilendarstellung zum Debuggen peripherer Schnittstellen.
			</p>
		</header>
		...
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:items-stretch">
			<Card class="border-border/80 group/card flex h-full min-h-0 flex-col shadow-sm ring-1 ring-black/5">
				<CardHeader>
					<CardTitle>Patientendaten</CardTitle>
					<CardDescription>FK&nbsp;3000, 3101, 3102, 3103, 3110 &mdash; editierbare Maske</CardDescription>
				</CardHeader>
				<CardContent class="grid flex-1 gap-x-4 gap-y-3 sm:grid-cols-2">
					<div class="flex min-h-0 flex-col gap-1">
						<label class="text-muted-foreground shrink-0 text-xs font-medium leading-none" for="pid">Patienten-ID</label>
						<input
							id="pid"
							type="text"
					...
					<div class="flex min-h-0 flex-col gap-1">
						<label class="text-muted-foreground shrink-0 text-xs font-medium leading-none" for="dob">
					...
					<div class="flex min-h-0 flex-col gap-1">
						<label class="text-muted-foreground shrink-0 text-xs font-medium leading-none" for="fn">Vorname</label>
						<input id="fn" type="text" bind:value={firstNameInput} autocomplete="given-name" class={fieldOneLineClass} />
					</div>
					<div class="flex min-h-0 flex-col gap-1">
						<label class="text-muted-foreground shrink-0 text-xs font-medium leading-none" for="ln">Nachname</label>
						<input id="ln" type="text" bind:value={lastNameInput} autocomplete="family-name" class={fieldOneLineClass} />
					</div>
					<div class="flex min-h-0 flex-col gap-1.5 sm:col-span-2">
						<span class="text-muted-foreground shrink-0 text-xs font-medium leading-none">Geschlecht (Schreib‑FK)</span>
					...
					{#if lastParsedApi}
						<div class="text-muted-foreground sm:col-span-2 text-xs leading-relaxed">
							Interpretiertes FK-Geschlecht:
							<Badge variant="outline" class="border-primary/35 bg-primary/8 text-primary font-medium">
								{lastParsedApi.gender}
							</Badge>
							{#if lastParsedApi.birthDate}
								&nbsp;&middot;&nbsp;<span>Parsed DOB:&nbsp;<span class="font-mono">
			...
			<Card class="border-border/80 group/card flex h-full min-h-0 flex-col shadow-sm ring-1 ring-black/5">
				<CardHeader class="border-border/60 border-b pb-4">
					...
					<CardDescription>Binärneutraler latin1‑Text&nbsp;&mdash;&nbsp;hier bearbeitbar</CardDescription>
				</CardHeader>
				<CardContent class="flex min-h-0 flex-1 flex-col gap-4">
					...
					<div class="border-border/70 bg-muted/25 rounded-lg border px-3 py-2.5">
						<label class="text-foreground flex cursor-pointer items-start gap-2.5 text-sm leading-snug">
							<input
								type="checkbox"
							...
							/>
							<span class="text-muted-foreground select-none">Strenges Parsen (/api/read?strict=1)</span>
						</label>
					</div>
					<Textarea
		...
		<Tabs bind:value={tab} class="flex flex-col gap-4">
			...
			>
				...
				<TabsTrigger value="annotated">Annotierter Datensatz</TabsTrigger>
			</TabsList>

			<TabsContent value="befund" class="flex flex-col gap-4 outline-none">
				<Card class="border-border/80 shadow-sm ring-1 ring-black/5">
					<CardHeader>
						<CardTitle>Befunde (FK&nbsp;6228)</CardTitle>
						<CardDescription>Mehrzeiliger Langtext wird als mehrere FK-Lines serialisiert</CardDescription>
					</CardHeader>
					<CardContent>
						<Textarea bind:value={befundeInput} rows={10} class="font-mono text-xs leading-snug" />
					</CardContent>
					<CardFooter class="flex flex-wrap gap-3">
						<Button class="inline-flex gap-2" onclick={roundTripViaServer} disabled={loading}>
							{#if loading}
			...
			<TabsContent value="annotated" class="flex flex-col gap-4 outline-none">
				<Card class="border-border/80 shadow-sm ring-1 ring-black/5">
					<CardHeader>
								...
								>
									<thead class="bg-muted/80 text-foreground sticky top-0 z-[1]">
										<tr>
											...
											<th class="text-left font-semibold">FK</th>
											<th class="text-left font-semibold">Benennung</th>
											<th class="text-right font-semibold">LLL</th>
											<th class="text-left font-semibold">Inhalt</th>
										</tr>
									</thead>
									<tbody>
										{#each annotatedLines as line (line.lineNumber)}
											<tr class="even:bg-muted/20 hover:bg-muted/40 transition-colors">
												<td class="text-muted-foreground font-mono">{line.lineNumber}</td>
												<td class="font-mono">{line.fieldId}</td>
												<td class="leading-snug">{line.label}</td>
												<td class="text-right font-mono">{line.declaredLength}</td>
												<td class="break-all whitespace-pre-wrap font-mono">
													{line.payloadPreview}
													{#if line.parseIssue}
														<span class="text-destructive block">{line.parseIssue}</span>
													{/if}
												</td>
											</tr>
										{/each}
